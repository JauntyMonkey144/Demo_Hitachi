/*
Name:        	WI_UI_Edit.js
Description: 	WI_UI_Edit js file containing logic for editing the WorkInstruction details and File.

Ver	Release	By					Date				Change Description
001	00.70		Fayaz A 		2024-09-05	#3460 First version.
002	00.70		Somya		  	2024-12-02	#3726 Review Comment Changes.
003	00.70		Chitta			2025-12-02	#4269 Promise all for async function will not work for dependency
																			control data mapping. dependency function for control must be sync.
004	01.00		Bas van B		2025-03-05	#4253 Translate entity and item descriptions MD.
005	01.00		Bas van B		2025-03-05	#4521 Updated function loadWorkInstructionData to load data Async.
006	01.01		Praveen 		2025-05-09	#4872 checking the validate of file type in fileupload control.
*/
((window) => {
	// ------------------------------ Global Variables ------------------------------------
	window.WI = window.WI || {};
	...
	function Edit() {
		...
		function initializeForm(Control) {
			...
			// _controls.attFileUpload = FORM.Control.findByXmlNode("ATFN");
			...
			_controls.txtLastEditComment = FORM.Control.findByXmlNode("TXLEC");
			_controls.hfFilePath = FORM.Control.findByXmlNode("HFFP");
			_controls.iwEdit = FORM.Control.findByXmlNode("IWE");
		...
		function ddWorkInstructionsTypeLoad() {
			const parameterColl = {};
			const data = FT.WebApi.mesGetSync("api/V3/DirectAccess", "SP_SA_WI_Instruction_Type", parameterColl, false);
		...
		function ddEntitiesLoad() {
			...
			const data = FT.WebApi.mesGetSync("api/V3/UserEntityAccess", "", parameterColl, false);
			if (data != null && data.length > 0) {
		...
		function ddItemListLoad() {
			...
			const data = FT.WebApi.mesGetSync(apiPath, "", parameterColl, false);
			if (data != null && data.length > 0) {
		...
		function ddBomVerListLoad() {
			const parameterColl = { itemId: _controls.ddItem.value };
			const data = FT.WebApi.mesGetSync("api/BomVersion", "", parameterColl, false);
			if (data != null && data.length > 0) {
				FT.WorkTasks.controlOptionsSetFromDataset("DDBV", 0, data, "ver_id", "ver_id");
				// _controls.ddBomVerId.value = selectedBomVer;
			}
		}
		...
		/**
		 * Load the selcted instruction values to the controls.
		 */
		function loadWorkInstructionData() {
			// get context value of instruction
				...
				FT.WebApi.mesGetAsync("api/V3/DirectAccess", "sp_S_WI_instruction", parameterCollection, false).then(
					(instructionData) => {
						if (instructionData.length > 0) {
							_controls.ddInstructionType.value = instructionData[0].type_id;
							_controls.ddEntity.value = instructionData[0].ent_id;
							_controls.ddItem.value = instructionData[0].item_id;
							_controls.ddBomVerId.value = instructionData[0].bom_ver_id;
							_controls.txtFileName.value = instructionData[0].file_name;
							_controls.hfFilePath.value = instructionData[0].file_name.replace(/\\/g, "\\\\");
							_controls.txtFileDesc.value = instructionData[0].file_desc;
							_controls.ddFileType.value = instructionData[0].file_type;
							_controls.hfRowId.value = instructionData[0].row_id;
							_controls.txtLastEditComment.value = instructionData[0].last_edit_comment;
						}
					},
				);
			}
		...
		/**
		 * Handles the attachment upload event and assigns the uploaded file name to a textbox.
		 */
		function onAttachmentUpload() {
			_controls.txtFileName.value = "";
			// Get the attachment control element
			// const controlElement = _controls.attFileUpload;
		...
		/**
		 * Handles the Edit instruction preWorkflow execution.
		 */
		function iwEditInstructionPreExec() {
			_controls.hfFilePath.value = _controls.txtFileName.value.replace(/\\/g, "\\\\");
		}
		/**
				...
				(data) => {
					if (data != null && data.length > 0) {
						if (data[0].param16 != null && data[0].param16 !== "") {
							...
							_controls.iwEdit.enable = true;
						} else {
							_controls.iwEdit.enable = false;
							SFU.showWarning(
		 ...
		 */
		function iwEditInstructionPostExec(blockingOutput, workflowStatus) {
		...
		return {
			...
			iwEditInstructionPreExec: iwEditInstructionPreExec,
			iwEditInstructionPostExec: iwEditInstructionPostExec,
