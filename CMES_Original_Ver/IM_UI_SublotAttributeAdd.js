/*
Name:        	IM_UI_SublotAttributeAdd.js
Description: 	IM_UI_SublotAttributeAdd js file containing logic pertaining to the Add Sublot Atttibutes form.

Ver		Release	    By				Date			    Change Description
001		01.02.00  	Praveen   2025-05-07		#5112 First version.
*/

((window) => {
	//  ------------------------------ Global Variables ------------------------------------
	window.IM = window.IM || {};
	IM.SublotAttributeAdd = IM.SublotAttributeAdd || {};
	IM.SublotAttributeAdd = SublotAttributeAdd();
	//  ------------------------------------------------------------------------------------
	/**
	 * formFunctions
	 *
		...
		function iwCreateSublotAttrOnPostWorkflow(blockingOutput, workflowStatus) {
			...
			if (workflowStatus === FT.WorkTasks.WF_STATUS.finishedSuccessfully) {
				FT.Common.windowEventDispatch(
					"im",
					"im.sublotAttrAdd.add",
					FT.Common.EVENT_SOURCE_TYPE.form,
					"IM_UI_SublotAttributeAdd",
					"im.sublotAttrAdd.add",
				);
			}
		}
