/*
Name:        	JM_UI_JobReady.js
Description: 	JM_UI_JobReady js file containing global logic pertaining to the JM_UI_JobReady Form.

Ver		Release		By			  Date			Change Description
001		00.50		Ramesh V	  2024-08-07	#2766	First version.
002		00.70		João Caldeira 2024-11-19	#3942 Updated form and file name from JM_UI_Ready to JM_UI_JobReady.
																					Added code to dispatch event on job ready.
...
((window) => {
	// ------------------------------ Global Variables ------------------------------------
	window.JM = window.JM || {};
	JM.JobReady = JM.JobReady || {};
	JM.JobReady = JobReady();
	// ------------------------------------------------------------------------------------
	/**
	 * formFunctions
	 *
	...
	function JobReady() {
				...
				FT.WebApi.mesGetAsync("api/v3/Jobs/key", "", parameterColl, false).then(
					(data) => {
						...
						if (data) {
							...
							_controls.hfRowId.value = "[" + jobDetails.row_id + "]";
							...
							_controls.hfJobStateCd.value =  FT.Common.MES_JOB_STATE_CD.ready;
						}
					},
					(error) => {
						// Handle error
		 ...
		 */
		function iwJobReadyOnPostWorkflow(blockingOutput, workflowStatus) {
			...
			if (workflowStatus === FT.WorkTasks.WF_STATUS.finishedSuccessfully) {
				FT.Common.windowEventDispatch("jm", "jm.job.ready", FT.Common.EVENT_SOURCE_TYPE.form, "JM_UI_JobReady", "jm.job.ready");
			}
		}
		/**
		...
		return {
			...
			iwJobReadyOnPostWorkflow: iwJobReadyOnPostWorkflow,
		};
	}
})(window);
