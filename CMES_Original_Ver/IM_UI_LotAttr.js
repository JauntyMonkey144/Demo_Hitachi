/*
Name:        	IM_UI_LotAttr.js
Description: 	IM_UI_LotAttr js file containing logic pertaining to the Add Lot Atttibutes form.

Ver		Release	By					Date					Change Description
001		01.00  	Praveen  		2025-02-07		#4314 First version.
...
((window) => {
	...
	function LotAttr() {
		...
		function onFormLoad() {
			...
			ddAttributeLoad();
			bindLotAttrDetails(attrid, attrvalue, note, JSON.parse(buttonAction).command);
		 ...
		 */
		function bindLotAttrDetails(attrid, attrvalue, note, command) {
		 ...
		 * Prepares data and sets control values before executing the workflow.
		 */
		function iwEditLotAttrOnPreWorkflow() {
			if (FORM.Control.validateForm() === true) {
		 ...
		 * Performs actions after the execution of a workflow.
		 */
		function iwEditLotAttrOnPostWorkflow(blockingOutput, workflowStatus) {
			FT.WorkTasks.workflowCheckStatus(blockingOutput, workflowStatus, true, 30000);
			if (workflowStatus === FT.WorkTasks.WF_STATUS.finishedSuccessfully) {
				FT.Common.windowEventDispatch(
					"im",
		...
		return {
			...
			iwEditLotAttrOnPreWorkflow: iwEditLotAttrOnPreWorkflow,
			iwEditLotAttrOnPostWorkflow: iwEditLotAttrOnPostWorkflow,
