/*
Name:        	OM_UI_Release.js
Description: 	OM_UI_Release js file containing global logic pertaining to the OM_UI_Release Form.

Ver		Release   By		    Date		Change Description
001		00.50.00  Shamanth S	2024-07-08	#3093	First version.
	...
	function Release() {
		...
		function onFormLoad() {
			...
			if (woContext && woContext.length > 0) {
				...
				_controls.hfStateCD.value = FT.Common.MES_WO_STATE_CD.released;
			}
		}
		/**
		 * Performs actions after the execution of a workflow.
		 */
		function iwOMReleasePostWorkflow(blockingOutput, workflowStatus) {
			...
			if (workflowStatus === FT.WorkTasks.WF_STATUS.finishedSuccessfully) {
				FT.Common.windowEventDispatch("om", "om.wo.release", FT.Common.EVENT_SOURCE_TYPE.form, "OM_UI_Release", "om.wo.release");
			}
		}
		/**
		...
		return {
			...
			iwOMReleasePostWorkflow: iwOMReleasePostWorkflow,
		};
	}
})(window);
