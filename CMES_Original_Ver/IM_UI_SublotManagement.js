/*
Name:        	IM_UI_SublotManagement.js
Description: 	IM_UI_SublotManagement js file containing logic pertaining to the Sublot Management form.

Ver		Release	  By					Date					Change Description
001		01.02.00  Praveen  		2025-06-18		#5052 First version.
*/

((window) => {
	//  ------------------------------ Global Variables ------------------------------------
	window.IM = window.IM || {};
	IM.SublotManagement = IM.SublotManagement || {};
	IM.SublotManagement = SublotManagement();
	//  ------------------------------------------------------------------------------------
	/**
	 * formFunctions
	 *
	...
	function SublotManagement() {
		...
		FORM.Control = null;
		const _controls = {};
		const IM_EVENTS = "im.SublotAdd.add|im.SublotEdit.edit";
		const IM_MODULE = "im";
		const IM_SUBLOT_ACTIONS_DATA =
			'[{"command":"details","param20":"","icon":"view--detail.svg","title":"Details","form_name":"IM_UI_SublotView"}' +
			',{"command":"add","param20":"","icon":"action--create.svg","title":"Add","form_name":"IM_UI_SublotAdd"}' +
			',{"command":"edit","param20":"","icon":"action--edit.svg","title":"Edit","form_name":"IM_UI_SublotEdit"}' +
		...
		function initializeForm(Control) {
			...
			_controls.wwSublot = FORM.Control.findByXmlNode("WWSLT");
		...
		function onFormLoad() {
			try {
				...
				FT.WorkTasks.contextSet("", "itemInv", JSON.stringify(SublotObj));
				lastFilterValue = "";
		...
		/**
		 * Function to loads lot data using 'sp_SA_IM_Sublot' with selected filters and updates the wwSublot control
		 * @param {string} item_class_list
		 * @param {string} grade_list
		 * @param {string} state_list
		 * @returns {JSON} data
		 */
		function wwSublotLoad() {
			...
			FT.WebApi.mesGetAsync("api/V3/DirectAccess", spName, parameterColl, false).then(
				(data) => {
					...
					_controls.wwSublot.widgetProperties.data = JSON.stringify(data);
		...
		/**
		 * Updates filter values on checkbox change and reloads sublot data if changed
		 */
		...
		/**
		 * Handles sublot row click: sets item context and triggers a UI update.
		 */
		function wwSublotonClick() {
			const selectedRowValue = _controls.wwSublot.widgetProperties.selectedRow;
			if (selectedRowValue) {
				_controls.epContainer.url = "";
				const sublotQueueRow = JSON.parse(_controls.wwSublot.widgetProperties.selectedRow);
				const SublotObj = [
					{
						rowId: sublotQueueRow.row_id,
						jsonValue: sublotQueueRow,
					},
				];
				FT.WorkTasks.contextSet("", "itemInv", JSON.stringify(SublotObj));
