/*
Name:        	IM_UI_LotEdit.js
Description: 	IM_UI_LotEdit js file containing logic pertaining to the edit Lot form.

Ver		Release   	By					Date					Change Description
001		01.02.00  	Praveen  		2025-06-18		#5096 First version.
*/

((window) => {
	//  ------------------------------ Global Variables ------------------------------------
	window.IM = window.IM || {};
	IM.LotEdit = IM.LotEdit || {};
	IM.LotEdit = LotEdit();
	//  ------------------------------------------------------------------------------------
	/**
	 * formFunctions
	...
	function LotEdit() {
		...
		function bindLotDetails(itemId, lotno) {
			const parameterColl = { itemId: itemId, lotNo: lotno };
			const lotDetails = FT.WebApi.mesGetSync("api/V3/Lot/key", "", parameterColl, false);
			if (lotDetails) {
				// Handle successful response data
				_controls.ddItemDesc.value = lotDetails.item_id;
				_controls.txLot.value = lotDetails.lot_no;
				_controls.ddGrade.value = lotDetails.grade_cd;
				_controls.ddState.value = lotDetails.status_cd;
				_controls.dtExpriyDate.value =
					lotDetails.expiry_date !== null
						? FT.WorkTasks.dateTimeInStringFormat(_controls.dtExpriyDate, lotDetails.expiry_date)
						: "";
				_controls.txSpare1.value = lotDetails.spare1;
				_controls.txSpare2.value = lotDetails.spare2;
				_controls.txSpare3.value = lotDetails.spare3;
				_controls.txSpare4.value = lotDetails.spare4;
				_controls.txSpare5.value = lotDetails.spare5;
				_controls.txSpare6.value = lotDetails.spare6;
				_controls.txComment.value = lotDetails.last_edit_comment;
				_controls.blSeriano.defaultValue = lotDetails.is_serial_no === false ? "False" : "True";
			}
		}
		/**
		 ...
		 */
		function iwEditLotOnPostWorkflow(blockingOutput, workflowStatus) {
		...
		return {
			...
			iwEditLotOnPostWorkflow: iwEditLotOnPostWorkflow,
		};
	}
})(window);
