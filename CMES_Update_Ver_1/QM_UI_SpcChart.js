...
((window) => {
	...
	function SpcChart() {
		...
		function setWidgetData(cData) {
			...
			if (_controls.wwTimeFilter && _controls.wwTimeFilter.widgetProperties) {
				startDatetime = SFU.getDateTimeInServerUTCFormat(new Date(_controls.wwTimeFilter.widgetProperties.start));
				endDatetime =
					_controls.wwTimeFilter.value === "shift" || _controls.wwTimeFilter.widgetProperties.selected.toUpperCase() === "CUSTOM"
						? SFU.getDateTimeInServerUTCFormat(new Date(_controls.wwTimeFilter.widgetProperties.end))
						: SFU.getDateTimeInServerUTCFormat(new Date(currentTime));
				...
				defaultStart.setHours(defaultStart.getHours() - 8);
				startDatetime = SFU.getDateTimeInServerUTCFormat(defaultStart);
				endDatetime = SFU.getDateTimeInServerUTCFormat(currentTime);
			}

			chartData.startTimeUtc = startDatetime;
			chartData.endTimeUtc = endDatetime;

			if (_controls.SpcChartWidget && _controls.SpcChartWidget.widgetProperties) {
				_controls.SpcChartWidget.widgetProperties.data = chartData;
			} else {
				var backupChartCtrl = FORM.Control ? (FORM.Control.topLevelForm ? FORM.Control.topLevelForm.findByXmlNode("WSPC") : null) : null;
				if (backupChartCtrl && backupChartCtrl.widgetProperties) {
					backupChartCtrl.widgetProperties.data = chartData;
				} else {
					SFU.showError("Lỗi giao diện", "Không thể tìm thấy Widget biểu đồ mã 'WSPC' trên hệ thống Form.", null, "");
				}
			}
