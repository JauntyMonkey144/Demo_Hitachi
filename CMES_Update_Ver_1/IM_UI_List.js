...
((window) => {
	...
	function List() {
		...
		function wwGridOnClick() {
			_controls.hfIMRow.value = _controls.wwIMQueue.widgetProperties.selectedRow != null ? _controls.wwIMQueue.widgetProperties.selectedRow : "";
			...
			if (_controls.wwIMQueue.widgetProperties.selectedRow != null) {
				...
				// --- CẬP NHẬT: LƯU LOT VÀO HỘP THƯ CHUNG CỦA BROWSER ---
				if (selectedRow && selectedRow.lot_no) {
					window.top.MES_SELECTED_LOT = selectedRow.lot_no; // Lưu lại để dùng khi chuyển tab
					
					// Vẫn giữ event phòng trường hợp cả 2 tab cùng hiển thị song song
					window.dispatchEvent(new CustomEvent('im.itemInv.lotSelected', { 
						detail: { lot_no: selectedRow.lot_no } 
					}));
				}
				// -----------------------------------------------------

				const inventoryObj = [{ rowId: selectedRow.row_id_h, jsonValue: selectedRow }];
