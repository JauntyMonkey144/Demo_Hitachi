	...
	function Dashboard() {
		...
		function wwNavigationSetHeader() {
            try {
                const parameterCollection = {
                    ent_name: entName, user_id: mesUserId, nav_grp_id: NAVIGATON_GRPID,
                    type: "HeaderForm", category: category,
                };
                const data = FT.WebApi.mesGetSync("api/V3/DirectAccess", "sp_SA_SC_Config_Dashboard", parameterCollection, false);

                // IN RA XEM DATABASE TRẢ VỀ CÁI GÌ
                console.log(">>> [DEBUG] Dữ liệu Header từ DB:", data); 

                if (data.length > 0) {
                    // Nếu có cấu hình, lấy form từ DB
                    _controls.epHeader.visible = true;
                    // NẾU form_name TRẢ VỀ BỊ RỖNG, ÉP DÙNG FORM CỦA MÌNH
                    var formToLoad = data[0].form_name ? data[0].form_name : "CUS_UI_JobProgress_Full"; 
                    console.log(">>> [DEBUG] Header sẽ load form:", formToLoad);
                    _controls.epHeader.url = SFU.getFormUrl(formToLoad);
                } else {
                    console.log(">>> [DEBUG] Không có cấu hình DB. Đang ép load CUS_UI_JobProgress_Full");
                    _controls.epHeader.visible = true;
                    _controls.epHeader.url = SFU.getFormUrl("CUS_UI_JobProgress_Full"); 
                }
            } catch (exception) {
                handleScriptError(exception);
            }
		...
		function hfNavbarOnDataChange() {
			...
			if (_controls.hfNavbar.value !== "") {
				...
				console.log(">>> [DEBUG] Tab Selected Action Object:", selectedAction);
                console.log(">>> [DEBUG] Form Name to Load:", selectedAction ? selectedAction.form_name : "None");
				if (selectedAction) {
